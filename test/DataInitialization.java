...
public class DataInitialization {
	...
	DataInitialization() throws SQLException
	{
		...
		try { 
			...
			String createTable = "CREATE TABLE Product(stock_number CHAR(7), Price REAL, manufacturer_name CHAR(20), model_number CHAR(20), category CHAR(20), warranty INTEGER, PRIMARY KEY (stock_number))";
			ResultSet rs = st.executeQuery(createTable);
			System.out.println("Product table created");
			
			//Accessory Table
			createTable = 	"CREATE TABLE Accessory(parent_stock_number CHAR(7), child_stock_number CHAR(7), PRIMARY KEY(parent_stock_number, child_stock_number), FOREIGN KEY(parent_stock_number) REFERENCES Product(stock_number), FOREIGN KEY(child_stock_number) REFERENCES Product(stock_number))";
			rs = st.executeQuery(createTable);
			System.out.println("Accessory table created");
		
			//Description Table
			createTable = "CREATE TABLE Description(attribute CHAR(20), value CHAR(20), stock_number CHAR(7), PRIMARY KEY (stock_number, attribute), FOREIGN KEY (stock_number) REFERENCES Product(stock_number))";
			rs = st.executeQuery(createTable);
			System.out.println("Description table created");

			//Customer Table
			createTable = "CREATE TABLE Customer(customer_ID CHAR(20), password CHAR(20), name CHAR(20), email CHAR(20), address CHAR(30), isManager INTEGER, status CHAR(20), PRIMARY KEY (customer_ID))";
			rs = st.executeQuery(createTable);
			System.out.println("Customer table created");

			//Shopping Cart Table
			createTable = "CREATE TABLE ShoppingCart(customer_ID CHAR(20), stock_number CHAR(20), quantity INTEGER, total_price REAL, PRIMARY KEY(customer_ID, stock_number), FOREIGN KEY(stock_number) REFERENCES Product, FOREIGN KEY(customer_ID) REFERENCES Customer)";
			rs = st.executeQuery(createTable);
			System.out.println("ShoppingCart table created");

			//Orders Table
			createTable = "CREATE TABLE Orders(order_number CHAR(20), sale_date DATE, customer_ID CHAR(20), stock_number CHAR(20), customer_Discount REAL, temp_cost REAL, final_cost REAL, PRIMARY KEY(order_number, stock_number, customer_ID), FOREIGN KEY(customer_ID) REFERENCES Customer, FOREIGN KEY(stock_number) REFERENCES Product)";
			rs = st.executeQuery(createTable);
			...
			//Product Depot Table
			createTable = "CREATE TABLE ProductDepot(stock_number CHAR(7), manufacturer CHAR(20), model_number CHAR(20), quantity INTEGER, min_stock_level INTEGER, max_stock_level INTEGER, location CHAR(20), replenishment INTEGER, PRIMARY KEY(stock_number))";
			rs = st.executeQuery(createTable);
			System.out.println("ProductDepot table created");

			//Shipping Notice Table
			createTable = "CREATE TABLE ShippingNotice( shipID CHAR(20), shipping_company_name CHAR(20), PRIMARY KEY (shipID))";
			rs = st.executeQuery(createTable);
			System.out.println("ShippingNotice table created");

			//Created Shipped Items Table
			createTable = "CREATE TABLE ShippedItems(manufacturer CHAR(20),model_number CHAR(20), quantity INTEGER, shipID CHAR(20), PRIMARY KEY (manufacturer, model_number), FOREIGN KEY (shipID) REFERENCES ShippingNotice)";
			rs = st.executeQuery(createTable);
